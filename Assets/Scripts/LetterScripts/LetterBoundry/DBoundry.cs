...
public class DBoundry : MonoBehaviour
{
    public D D;
    ...
    void OnTriggerEnter2D(Collider2D activator){
        if(activator.tag == "TriggerBox1"){
            ...
            D.transform.position = position + new Vector3 (0f,0f,-1f);
            D.GetComponent<Rigidbody2D>().velocity = Vector3.zero; 
            D.GetComponent<Rigidbody2D>().angularVelocity = 0;
